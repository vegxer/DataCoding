...
namespace coding
{
    public static class Calc
    {
            ...
            else
            {
                ...
                if (IsDoubleNumber(answer, 10))
                {
                    if (answer.IndexOf('E') != -1)
                    {
                        ...
                        return ConvertToBase(answer.Substring(0, answer.IndexOf('e')), 10, numSystem, 1e-10) +
                            "e" + ConvertToBase(answer.Substring(answer.IndexOf('e') + 1), 10, numSystem, 1e-10);
                    }
                    return RemovePointlessZeros(ConvertToBase(RemovePointlessZeros(answer), 10, numSystem, 1e-10));
                }
                else
                    return "Ошибка";
            }
        ...
        /// <param name="eps">Точность перевода дробных чисел</param>
        /// <example>
        /// <code>
        ...
        /// </code>
        /// </example>
        public static string ConvertToBase(string number, int from, int to, double eps)
        {
            string decimalPart = "";
            ...
            try
            {
                if (from > 1 && from < 17 && to > 1 && to < 17)
                {
                    if (number.IndexOf(',') != -1)
                    {
                        decimalPart = ConvertFractionalPartToBase(number, from, to, eps);
        ...
        static string ConvertFractionalPartToBase(string number, int from, int to, double eps)
        {
            double decNumber = ConvertFractionalPartToDecimal(number.Substring(number.IndexOf(',') + 1), from);
            if (to == 10)
                return decNumber.ToString().Substring(1);

            double inputNumber = double.Parse(number);
            double currNumber = (int)inputNumber;
            string decimalPart = "";
            for (double decPart = decNumber; Math.Abs(inputNumber - currNumber) >= eps && decPart != 0; decPart -= (int)decPart)
            {
                decPart *= to;
                if ((int)decPart > 9)
                ...
                currNumber += (int)decPart * Math.Pow(to, -decimalPart.Length);
            }
        ...
        static void ConvertOperandsToDecimal(string[] operands, int numSystem)
        {
            for (int i = 0; i < operands.Length; ++i)
                operands[i] = ConvertToBase(operands[i], numSystem, 10, 1e-10);
        }
        ...
        {
            if (str.IndexOf('π') != -1)
                str = str.Replace("π", ConvertToBase(Math.PI.ToString(), 10, numSystem, 1e-10));
            if (str.IndexOf('e') != -1)
                str = str.Replace("e", ConvertToBase(Math.E.ToString(), 10, numSystem, 1e-10));
            str = str.Replace(" ", "");
